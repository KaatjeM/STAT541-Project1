...
These data are important to measure and evaluate the differences across regimes, and evaluate changes in democracies on a global scale.
...
The following code up to Checkpoint 4 will not evaluate due to long runtimes.
```{r}
#| eval: false
...
```{r}
# Load in packages needed for map geometries
library(sf)
...
# Load in master dataset from Checkpoints 1-3
...
# load in packages needed for sankey plot
...
## Statistical Model - Time Until Country Returns To Democracy

This survival analysis will assess the cumulative probability of *returning* to a democratic regime for those that converted from a democratic regime to a non-democratic regime (e.g., by coup d'état).

We'll call these non-democratic intervals "spells" and measure the time until non-democratic spells are broken. Note that some countries are left-censored since we only have data going back to 1950 – those countries may have been non-democratic prior to 1950 and the model doesn't reflect this.
