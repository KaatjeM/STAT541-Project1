...
combined |>
  ...
  geom_sf(aes(fill = factor(is_democracy,
                            levels = c(FALSE, TRUE),
                            labels = c("No", "Yes"))), linewidth = 0.05) +
...
## Model - Time Until Country Returns To Democracy
...
title(main = "Prob. of Remaing Non-Democratic Decreases Over Time", adj = 0)
```
...
cox_model_new <- coxph(
  Surv(time, returned_to_democracy) ~
    is_communist +
    is_multiparty +
    strata(region) +
    cluster(country_code),
...
- n = 255 spells, 178 returns to democracy

- Model significantly predicts "time to democracy" (p \< 2e-16)

- Hazard Ratio Interpretations

  - Europe & Central Asia: HR = 2.67
  ...
  - Latin America & Caribbean: HR = 3.31
  ...
  - Middle East & North Africa: HR = 0.036
